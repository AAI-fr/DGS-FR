# The Chronicles of Ace Attorney
...
## Avancée
### 3DS :
Le premier jeu est entièrement traduit. Le travail est encore en court sur les DLCs, mais le premier d'entre eux est disponible.
### HD (Steam, Switch) :
...
À noter que le patcheur nécessite des versions de systèmes d'exploitations récentes. Nous essaierons de travailler sur la compatibilité dans le futur. En cas de problème, n'hésitez pas à nous contacter.

## Instructions
### Nintendo 3DS :
#### Prérequis :
- Une Nintendo 3DS hackée.
- Une ROM japonaise du jeu déchiffrée (.cia ou .3ds).
- Une ROM DLC japonaise du jeu déchiffrée (.cia, facultative mais fortement recommandée).

Une fois le patch terminé, vous devriez voir les ROM patchées au format .cia dans le même dossier que le patcheur. Mettez-les sur la carte SD de votre 3DS et installez-les avec FBI.
...
- Les clefs de déchiffrements du firmware de la Switch (fichier communément appelé prod.keys).
- Une ROM du jeu (.nsp), que ce soit la version japonaise ou la version internationale. (Note : la version japonaise n'a été que très peu testée et n'est donc pas recommandée).
